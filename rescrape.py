...
Address = StreetNumber + ' ' + StreetName + ' ' + AptNumber + ' ' + City + ' ' + State + ' ' + ZipCode
...
while True:
    try:
        ...
        print('ERROR!!! The user supplied address cannot be GeoCoded with Nominatim.\n'
        ...
        Address = input("Try typing in the specific physical address using method above: ")
...
NorthDestination = geodesic(kilometers=(SearchBoxHalfWidth * 1.60934)).destination(origin, 0.0)
NorthBoundary = NorthDestination.latitude
EastDestination = geodesic(kilometers=(SearchBoxHalfWidth * 1.60934)).destination(origin, 90.0)
EastBoundary = EastDestination.longitude
SouthDestination = geodesic(kilometers=(SearchBoxHalfWidth * 1.60934)).destination(origin, 180.0)
SouthBoundary = SouthDestination.latitude
WestDestination = geodesic(kilometers=(SearchBoxHalfWidth * 1.60934)).destination(origin, 270.0)
...
ZillowURL = ['https://www.zillow.com/homes/recently_sold/?searchQueryState={'
             '"pagination":{},'
             '"mapBounds":{'
...
if ZillowPageList is None:
    ...
    print(ZillowPageLinks[-2].text, 'pages of Zillow search results exist in this search box.')
    NumZillowPages = int(ZillowPageLinks[-2].text)

# Find the total number of Zillow results, and store as an integer.
ZillowResultCount = int((ZillowHTML.find('span', class_='result-count')).text.split()[0].replace(',', ''))
print(ZillowResultCount, 'recently sold homes on Zillow were found within the search box.')
...
if ZillowResultCount > 800:
    print('Warning! The maximum number of pages Zillow shows is 20 and the maximum number of results per page is 40. \n'
...
for i in range(0, NumZillowPages):
    ...
    if i == 0:
        ...
        # 9/18/20 - having to change below code for new zillow html
        # old code # SoldHomeZillowLinkInfo = ZillowHTML.find_all("a", class_="list-card-link")
        ...
        for j in range(0, min(ZillowResultCount, 40)):
            ...
            # 9/18/20 - having to update code for new zillow html
            # old code # SoldHomeZillowLinks.append(SoldHomeZillowLinkInfo[j]['href'])
            SoldHomeZillowLinks.append(SoldHomeZillowLinkInfo[j].find("a", class_="list-card-link")['href'])
        ...
        ZillowURL.append('https://www.zillow.com/homes/recently_sold/' +
                         str(i+1) + '_p/?searchQueryState={'
        ...
        for Link in SoldHomeZillowLinkInfo:
            # 9/18/20 - having to update code for new zillow html
            # old code # SoldHomeZillowLinks.append(Link['href'])
...
if len(SoldHomeZillowLinks) == ZillowResultCount:
    print('The number of Sold Home Zillow Links scraped from all pages equals \n'
          'the Zillow Result Count that was scraped from the initial page.')
else:
    print("WARNING!!! The number of Sold Home Zillow Links scraped from all pages DOES NOT EQUAL \n"
...
print('{:^55s}{:^10s}{:>10s}{:>6s}{:^17s}{:^7s}{:^22s}{:^9s}{:^17s}{:^12s}'.format(
        'Address', 'Sell Price', 'Bed/Bath', 'SqFt', 'Type', 'Built', 'Heating', 'Cooling', 'Parking', 'Lot Size'))

for i in range(0, len(SoldHomeZillowLinks)):

    # A delay of 3 seconds between each request is instituted to ensure the Zillow servers are not overwhelmed.
    time.sleep(3)
    ...
    SoldHomeAddress = SoldHomeZillowLinks[i] \
    ...
    try:
        # 9/18/20 - having to change below code for new zillow html
        # SoldHomePrice = (ZillowHTML.find("div", class_="status")).text.replace(' Sold: $', '').replace(',', '')
        SoldHomePrice = ZillowHTML.find("div", class_="ds-home-details-chip").find("p").text.split()[1]\
            .replace('$', '').replace(',', '').replace('Sold', '')
    except AttributeError:
        SoldHomePrice = "n/a"
    ...
    # and/or sqft.
    try:
        # 9/20/2020 - having to change below code due to new zillow html
        # https://stackoverflow.com/questions/58631938/how-to-ignore-attributes-with-no-value-and-ignore-tags-with-specific-class-with
        # Home_Size_items = ZillowHTML.find("h3", class_="edit-facts-light").text.split()
        Home_Size_container = ZillowHTML.find("h3", class_="ds-bed-bath-living-area-container")
        Home_Size_items = []
        for item in Home_Size_container.select('.ds-bed-bath-living-area span:not(.ds-vertical-divider)'):
    ...
    try:
        SoldHomeFactLabels = ZillowHTML.find("ul", class_="ds-home-fact-list")\
            .find_all("span", class_="ds-home-fact-label")
        SoldHomeFactValues = ZillowHTML.find("ul", class_="ds-home-fact-list")\
            .find_all("span", class_="ds-home-fact-value")
    except AttributeError:
        SoldHomeType = "n/a"
    ...
    # thrown, the following else statement is executed.
        ...
        try:
            index = [j for j, x in enumerate(SoldHomeFactLabels) if 'type' in x.text.lower()]
            SoldHomeType = SoldHomeFactValues[index[0]].text
        except IndexError:
            SoldHomeType = "n/a"
        try:
            index = [j for j, x in enumerate(SoldHomeFactLabels) if 'year built' in x.text.lower()]
            SoldHomeYearBuilt = SoldHomeFactValues[index[0]].text
        except IndexError:
            SoldHomeYearBuilt = "n/a"
        try:
            index = [j for j, x in enumerate(SoldHomeFactLabels) if 'heating' in x.text.lower()]
            SoldHomeHeating = SoldHomeFactValues[index[0]].text
        except IndexError:
            SoldHomeHeating = "n/a"
        try:
            index = [j for j, x in enumerate(SoldHomeFactLabels) if 'cooling' in x.text.lower()]
            SoldHomeCooling = SoldHomeFactValues[index[0]].text
        except IndexError:
            SoldHomeCooling = "n/a"
        try:
            index = [j for j, x in enumerate(SoldHomeFactLabels) if 'parking' in x.text.lower()]
            SoldHomeParking = SoldHomeFactValues[index[0]].text
        except IndexError:
            SoldHomeParking = "n/a"
        try:
            index = [j for j, x in enumerate(SoldHomeFactLabels) if 'lot' in x.text.lower()]
            SoldHomeLotSize = SoldHomeFactValues[index[0]].text
        except IndexError:
            SoldHomeLotSize = "n/a"

    # Print the results of the scraping to the screen
    print('{:55s}{:>2s}{:>8s}{:>4s}{:^3s}{:3s}{:>6s}'
          '{:^17}{:^7}{:^22}{:^9}{:^17}{:^12}'.format(
            SoldHomeAddress, '$', SoldHomePrice, SoldHomeBeds, '/', SoldHomeBaths, SoldHomeSqFt,
            SoldHomeType, SoldHomeYearBuilt, SoldHomeHeating, SoldHomeCooling, SoldHomeParking, SoldHomeLotSize)
          )
